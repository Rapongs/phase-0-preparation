...
function missingNum(arr) {
  ...
  let sortedArr = flattenArr.sort((a,b) => a-b);
  let cleanedArr = sortedArr.filter(item => item !== " ");
  let missingArr = [];

  for (let i = cleanedArr[0]; i <= cleanedArr[cleanedArr.length - 1]; i++)
  {
    if (!cleanedArr.includes(i))
    {
        missingArr.push(i);
    }
  }

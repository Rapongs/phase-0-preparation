...
function dividableRecursive(array, num) {
  ...
  if (currentElement % num === 0) {
    let nextResult = dividableRecursive(remainingArray, num);
    
    return currentElement + (nextResult ? " " + nextResult : "");
  } else {
    return dividableRecursive(remainingArray, num);
  }
}
